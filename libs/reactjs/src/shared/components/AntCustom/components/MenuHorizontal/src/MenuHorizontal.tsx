...
import './styles.css';
import { handleAssignClassNamesToItems } from './utils/handleAssignClassNamesToItems';

export interface SubMenuItem<Key extends string> {
  /** The unique key for the submenu item. */
  key: Key;
  /** The label of the submenu item. */
  label: ReactNode;
  /** The children items of the submenu item. */
  children?: Array<MenuGroup<Key> | SubMenuItem<Key>>;
...
export const MenuHorizontal = <Key extends string>({
  ...
  const items_ = useMemo(() => {
    return handleAssignClassNamesToItems(items);
  }, [items]);
