...
export const Header = ({ onCreate, onExport, onImport, isExporting, creatable, exportable, importable }: Props) => {
  ...
  return (
    <HeaderListing
      ...
      onExport={onExport}
      onImport={onImport}
      title={t('contract_template:contract_templates')}
      createBtn={t('contract_template:add_new')}
      exportBtn={t('contract_template:export')}
      importBtn={t('contract_template:import')}
    />
  );
};
